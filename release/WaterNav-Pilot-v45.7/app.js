...
L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png',{maxZoom:19,keepBuffer:4,updateWhenIdle:false,updateWhenZooming:false,attribution:'Â© OpenStreetMap'}).addTo(map);
...
init();
function init(){migrateUserData();bindUI();loadUserSettings();loadSavedRoutes();renderSavedRoutes();loadHomePort();loadOrientationPreference();updateManifestDependentControls();loadTileManifest().then(()=>updateVisibleMapTiles({initial:true})).then(()=>setStatus('v45.7 klar. DDM tiles loader dynamisk for synligt kortomrÃ¥de og ruter.')).catch(e=>{console.error(e);setStatus(manifestErrorMessage(e));});updateInfoBox();cleanupOldCaches();if('serviceWorker'in navigator)navigator.serviceWorker.register('./sw.js?v=45.7').then(r=>r.update()).catch(()=>{});setTimeout(()=>{map.invalidateSize(true);applyMapOrientation(true);scheduleVisibleTileUpdate();},300)}
function bindUI(){
...
function onGps(pos){
 ...
 $('sogText').textContent=`SOG: ${state.lastSogKn?state.lastSogKn.toFixed(1):'â€”'} kn`;
 $('cogText').textContent=`COG: ${Number.isFinite(state.lastCog)?Math.round(state.lastCog):'â€”'}Â°`;
 updateBoatMarker();
 updateInfoBox();
...
function setPickMode(m){state.pickMode=m;setStatus(m==='start'?'Klik startpunkt pÃ¥ kortet.':m==='end'?'Klik slutpunkt pÃ¥ kortet.':m==='home'?'Klik din hjemhavn pÃ¥ kortet.':'Klar.')}
...
function loadUserSettings(){
 try{const s=JSON.parse(localStorage.getItem(USER_SETTINGS_KEY)||'{}');state.depthAlarm=s.depthAlarm!==false;state.offRouteAlarm=s.offRouteAlarm!==false;state.routeSmoothing=ROUTE_SMOOTHING_ENABLED&&ROUTE_SMOOTHING_VALUES.has(s.routeSmoothing)?s.routeSmoothing:'off';}catch{state.depthAlarm=true;state.offRouteAlarm=true;state.routeSmoothing='off'}
...
function saveUserSettings(){
 localStorage.setItem(USER_SETTINGS_KEY,JSON.stringify({depthAlarm:state.depthAlarm,offRouteAlarm:state.offRouteAlarm,routeSmoothing:state.routeSmoothing,updated:new Date().toISOString()}));
}
...
function exportUserData(){
 const payload={version:VERSION,exported:new Date().toISOString(),homePort:state.homePort,routes:state.savedRoutes,orientationMode:state.orientationMode,userSettings:{depthAlarm:state.depthAlarm,offRouteAlarm:state.offRouteAlarm,routeSmoothing:state.routeSmoothing}};
...
function importUserData(evt){
 ...
 if(data.userSettings){state.depthAlarm=data.userSettings.depthAlarm!==false;state.offRouteAlarm=data.userSettings.offRouteAlarm!==false;state.routeSmoothing=ROUTE_SMOOTHING_ENABLED&&ROUTE_SMOOTHING_VALUES.has(data.userSettings.routeSmoothing)?data.userSettings.routeSmoothing:'off';saveUserSettings();loadUserSettings();}
...
function updateOrientationText(){
 ...
 const text=state.orientationMode==='course'?(cog!==null?`Sejlretning op Â· COG ${cog}Â°`:'Sejlretning op Â· venter pÃ¥ COG'):'Nord op';
 if($('orientationBadge'))$('orientationBadge').textContent=text;
 if($('orientationHelp'))$('orientationHelp').textContent=state.orientationMode==='course'?'Kortet roterer efter COG, sÃ¥ din aktuelle sejlretning er op. Ved lav fart/ukendt COG holdes seneste sikre retning.':'Nord op: kortet er lÃ¥st mod geografisk nord. BÃ¥dikonen og kurslinjen viser stadig COG.';
}
...
function setTrollingMode(enabled,silent=false){
 ...
 if($('makeRouteMain'))$('makeRouteMain').textContent=state.trollingEnabled?'Lav trollingstrÃ¦kning':'Lav fri rute';
 if($('freeRouteControls'))$('freeRouteControls').hidden=state.trollingEnabled;
 if($('modeHelp'))$('modeHelp').textContent=state.trollingEnabled?'TrollingMode laver en strÃ¦kning langs valgt DDM-isobath. Brug Trolling-menu til dybde, tolerance, gemte trollingruter og vend retning.':'Fri navigation bruger minimumsdybde og beregner en water-only rute uden land/ukendt DDM-data.';
 updateDepthLabels();
 saveUserSettings();if(!silent)setStatus(state.trollingEnabled?'TrollingMode slÃ¥et til.':'TrollingMode slÃ¥et fra. Fri dybdestyret navigation aktiv.');
 renderContours();
...
function setRouteSmoothing(mode,silent=false){
 ...
 updateRouteSmoothingUi();
 saveUserSettings();
 if(!silent)setStatus(`RuteudjÃ¦vning: ${routeSmoothingLabel(state.routeSmoothing)}.`);
}
...
function routeSmoothingLabel(mode){return mode==='off'?'Fra':mode==='high'?'HÃ¸j':'Normal'}
...
function updateDepthLabels(){
 ...
 $('toleranceLabel').textContent='Tolerance / nÃ¦rmeste kurve';
 $('depthModeHelp').textContent=`TrollingMode fÃ¸lger DDM-isobath omkring ${d.toFixed(1)} m med nÃ¦rmeste kurve inden for Â±${tol.toFixed(2)} m.`;
...
function setStart(p){state.start={lat:p.lat,lng:p.lng};if(state.startMarker)state.startMarker.setLatLng(state.start);else state.startMarker=L.marker(state.start,{icon:startIcon,zIndexOffset:980}).addTo(map).bindPopup('Start');state.startMarker.bringToFront?.();updateRouteDebugUi();setStatus('Start valgt. VÃ¦lg slutpunkt.')}
function setEnd(p){state.end={lat:p.lat,lng:p.lng};if(state.endMarker)state.endMarker.setLatLng(state.end);else state.endMarker=L.marker(state.end,{icon:endIcon,zIndexOffset:990}).addTo(map).bindPopup('Slut');state.endMarker.bringToFront?.();updateRouteDebugUi();setStatus(state.trollingEnabled?'Slut valgt. Lav trollingrute pÃ¥ DDM-dybdekurven.':'Slut valgt. Lav fri rute efter valgt dybde.')}
function useGpsAsStart(){if(!state.gps)return setStatus('GPS er ikke klar endnu.');setStart(state.gps);map.panTo(state.gps)}
function centerOnGps(){if(!state.gps)return setStatus('GPS er ikke klar endnu.');map.panTo([state.gps.lat,state.gps.lng],{animate:true,duration:.3});setStatus('Kortet er centreret pÃ¥ aktuel GPS-position.');applyMapOrientation(true)}
...
function saveHomeFromGps(){
 if(!state.gps)return setStatus('GPS er ikke klar endnu, sÃ¥ hjemhavn kan ikke gemmes fra aktuel position.');
...
function renderHomePort(){
 ...
 const icon=L.divIcon({className:'home-marker',html:'<div class="home-dot">âŒ‚</div>',iconSize:[30,30],iconAnchor:[15,15]});
...
function showMapActionMenu(p){
 ...
 wrap.innerHTML=`<div class="mapActionMenuHeader"><strong>Valgt punkt</strong><button type="button" data-act="close" aria-label="Luk kortmenu">&times;</button></div><small>${p.lat.toFixed(5)}, ${p.lng.toFixed(5)} Â· DDM dybde: ${depthText}</small><button type="button" data-act="nav">NavigÃ©r hertil</button><button type="button" data-act="start">Brug som startpunkt</button><button type="button" data-act="end">Brug som slutpunkt</button><button type="button" data-act="home">Gem som hjemhavn</button><button type="button" data-act="troll">Start trollingstrÃ¦k her</button>`;
 ...
 wrap.querySelector('[data-act=troll]').onclick=()=>{setTrollingMode(true,true);setStart(p);map.closePopup();setStatus('TrollingMode: start valgt. VÃ¦lg slutpunkt pÃ¥ kortet.');beginMapPick('end');};
...
async function loadTileManifest(){
 setManifestStatus('loading',`DDM manifest: indlÃ¦ser... (${TILE_MANIFEST_PATH})`);
 ...
 $('dataSource').textContent='DDM 2024 tile-manifest Â· dynamisk loading';
 $('contourCount').textContent='0';
 setManifestStatus('ready','DDM manifest: indlÃ¦st');
 updateMapRegionsUi();
...
function requireTileManifest(actionLabel='Denne handling'){
 ...
 const msg=state.manifestError||`${actionLabel} krÃ¦ver DDM manifest. Forventet fil: ${expectedManifestPath()}`;
 setStatus(msg);
 return false;
}
...
async function loadCollisionGrid(){
 ...
 try{
  ...
  state.collisionGridError=`DDM collision-grid kunne ikke indlÃ¦ses (${FINE_COLLISION_GRID_PATH}): ${e?.message||e}`;
...
function tileFailureMessage(rec=state.lastTileError){
 if(!rec)return 'Nogle DDM-kortdata kunne ikke indlÃ¦ses';
 return `Tile fejl: ${rec.file} Â· ${rec.status}`;
}
...
function updateTileProgressUi(tracker){
 ...
 const message=complete?(tracker.failed?tileFailureMessage():'DDM-kortdata klar'):`IndlÃ¦ser DDM-kortdata: ${pct}%`;
...
async function reloadVisibleTiles(){
 if(!requireTileManifest('GenindlÃ¦s synlige DDM tiles'))return;
 ...
 setStatus('Synlige DDM-kortomrÃ¥der er genindlÃ¦st.');
}
...
async function updateVisibleMapTiles(opts={}){
 ...
 if(!ids.size){
  $('contourCount').textContent='0';
  $('activeDepth').textContent='Ingen DDM tiles i synligt kortomrÃ¥de';
  updateMapRegionsUi();
  return;
 }
...
async function prepareRoutingGrid(start,end){
 ...
 if(!pointInsideGrid(start,state.depthGrid)||!pointInsideGrid(end,state.depthGrid)){state.lastTileLoadError='Manglende DDM tile: start/slut ligger uden for installeret DDM-omrÃ¥de.';return false;}
 ...
 if(!tiles.length){state.lastTileLoadError='Manglende DDM tile for ruteomrÃ¥det.';return false;}
 const ids=new Set(tiles.map(tile=>tile.id));
...
function updateMapRegionsUi(){
 ...
 for(const region of state.tileManifest.regions||[]){
  ...
  card.innerHTML=`<div><strong>${escapeHtml(region.name)}</strong><small>${region.installed?'Installeret':'Ikke installeret'} Â· ${region.tileIds.length} tiles Â· ${loaded} loaded${visible?` Â· ${visible} synlige`:''}</small></div><button type="button" disabled>Download senere</button>`;
...
function updateDdmDebugUi(){
 ...
 const manifestText=state.manifestStatus==='ready'?'OK':state.manifestStatus==='loading'?'indlÃ¦ser':state.manifestStatus==='error'?'fejl':'venter';
...
function importGeoJsonFile(evt){const f=evt.target.files?.[0];if(!f)return;const r=new FileReader();r.onload=()=>{try{const geo=JSON.parse(String(r.result));const contours=parseContours(geo);if(!contours.length)return setStatus('GeoJSON indeholder ingen lÃ¦sbare dybdekurver.');setContours(contours,`importeret: ${f.name}`);setStatus(`Importeret ${contours.length} dybdekurver fra ${f.name}.`)}catch(e){setStatus('Kunne ikke lÃ¦se GeoJSON.')}};r.readAsText(f)}
...
function renderContours(){state.contourLayerGroup.clearLayers();state.activeContours=[];const contours=loadedContours();$('contourCount').textContent=String(contours.length);if(!contours.length){$('activeDepth').textContent='Ingen DDM-kurver i synligt kortomrÃ¥de';return;}const target=selectedDepth(),tol=selectedTolerance();let bestDiff=Infinity;for(const c of contours)bestDiff=Math.min(bestDiff,Math.abs(c.depth-target));const limit=Math.max(tol,bestDiff+.001);for(const c of contours){const d=Math.abs(c.depth-target);const active=d<=limit;const color=active?'#ffe100':'#188cff';const weight=active?4.4:1.15;const opacity=active?.96:.40;if(active)state.activeContours.push(c);const isCoarse=c.scope==='denmark';const visibleBase=[1,2,3,5,7,8,10,15,20,30,50].includes(Number(c.depth));if(active||visibleBase){const w=isCoarse?Math.max(0.8,weight*.65):weight;const op=isCoarse?(active?.75:.22):opacity;L.polyline(c.points.map(p=>[p.lat,p.lng]),{color,weight:w,opacity:op,smoothFactor:isCoarse?1.2:.55}).bindTooltip(`${c.depth.toFixed(1)} m Â· DDM${isCoarse?' Danmark coarse':''}`,{sticky:true}).addTo(state.contourLayerGroup)}}$('activeDepth').textContent=state.activeContours.length?`${target} m (${state.activeContours.length} DDM-kurver / nÃ¦rmeste Â±${limit.toFixed(2)}m)`:`Ingen DDM-kurve tÃ¦t pÃ¥ ${target} m i loaded tiles`}
...
async function makeTrollingRoute(){
 if(!state.start||!state.end)return handleRoutingFailure('VÃ¦lg start og slut fÃ¸rst.');
 if(!requireTileManifest('Lav trollingrute'))return;
 if(!await prepareRoutingGrid(state.start,state.end))return handleRoutingFailure(state.lastTileLoadError||'Manglende DDM tile eller start/slut uden for installeret DDM-omrÃ¥de.',{routingMode:'trolling',routingSource:'DDM grid',gridResolutionM:gridResolutionMeters(),routingTileCount:state.routingTileIds.size,fallbackUsed:false});
 ...
 state.currentRoute={id:'r_'+Date.now(),name:'',mode:'trolling',depth:target,actualDepth:result.avgDepth,points:result.points,created:new Date().toISOString(),lengthNm:pathNm(result.points),source:'Danmarks Dybdemodel 2024 ddm_50m.dybde Â· water-only A* routing',stats:result.stats};
 drawRoute(result.points,target,`TrollingMode Â· DDM vandrute omkring ${target.toFixed(1)} m`);
...
async function makeFreeRoute(){
 ...
 if(!await prepareRoutingGrid(state.start,state.end))return handleRoutingFailure(state.lastTileLoadError||'Manglende DDM tile eller start/slut uden for installeret DDM-omrÃ¥de.',{routingMode:'free',routingSource:'DDM grid',gridResolutionM:gridResolutionMeters(),routingTileCount:state.routingTileIds.size,fallbackUsed:false});
 ...
 if(!result.ok)return handleRoutingFailure(result.message,result.stats||{});
 state.currentRoute={id:'r_'+Date.now(),name:'',mode:'free',depth:target,minDepth,actualDepth:result.avgDepth,points:result.points,created:new Date().toISOString(),lengthNm:pathNm(result.points),source:'Danmarks Dybdemodel 2024 ddm_50m.dybde Â· depth-aware water-only A* routing',stats:result.stats};
 drawRoute(result.points,target,`Fri navigation Â· DDM vandrute â‰¥${minDepth.toFixed(1)} m / mÃ¥l ${target.toFixed(1)} m`);
 logRoutingSuccess(state.currentRoute,'free');
 updateRouteActionButtons();$('routeLength').textContent=`${state.currentRoute.lengthNm.toFixed(2)} NM`;
 setStatus(`Fri DDM-vandrute fundet. ${smoothingStatusText(result.stats)} Valgt minimumsdybde ${minDepth.toFixed(1)} m. Verificeret minimum pÃ¥ ruten ${result.stats.minDepth.toFixed(1)} m. Ruten krydser ikke land/for lavt vand. Grid: ${routingGridLabel()}.`)
}

function routingGridLabel(){return state.depthGridSource||'DDM tiles'}
function smoothingStatusText(stats){return `RuteudjÃ¦vning ${routeSmoothingLabel(stats?.smoothingMode||state.routeSmoothing)}: ${stats?.originalPointCount||0} â†’ ${stats?.smoothedPointCount||0} punkter (${Math.round(stats?.smoothingReductionPct||0)}%).`}
...
function buildWaterRoute(start,end,opts){
 ...
 if(!sCell)return{ok:false,message:'Startpunktet ligger ikke tÃ¦t pÃ¥ sejlbart DDM-vand i gridomrÃ¥det.'};
 if(!eCell)return{ok:false,message:'Slutpunktet ligger ikke tÃ¦t pÃ¥ sejlbart DDM-vand i gridomrÃ¥det.'};
 const search=aStarGrid(sCell,eCell,opts);
 const baseStats=routingBaseStats(opts,sCell,eCell,search);
 if(!search.reached||!search.path||search.path.length<2)return{ok:false,message:`Ingen vandrute fundet med minimum ${Number(opts.minDepth||0).toFixed(1)} m. PrÃ¸v lavere minimumsdybde eller flyt start/slut til dybere vand.`,stats:baseStats};
...
function validateSmoothedRouteOutput(start,end,points,opts){
 ...
 if(!Array.isArray(points)||points.length<=2)return{ok:false,message:'RuteudjÃ¦vning blev afvist: for fÃ¥ route points.',stats};
 ...
 if(startDist>routeEndpointMaxNm()||endDist>routeEndpointMaxNm())return{ok:false,message:'RuteudjÃ¦vning blev afvist: ruten nÃ¥r ikke destinationen.',stats};
...
function validateCompleteRouteOutput(start,end,path,points,opts,search){
 ...
 if(!Array.isArray(path)||!Array.isArray(points)||points.length!==path.length||points.length<=2)return{ok:false,message:'Ruten kunne ikke fÃ¸res helt til destinationen',stats:{...stats,pointCount:points?.length||0}};
 const first=points[0],last=points[points.length-1];
 const startDist=directNm(start,first),endDist=directNm(end,last);
 stats.startDistanceNm=startDist;
 stats.lastDistanceNm=endDist;
 stats.lastPoint=last;
 if(!search?.reached)return{ok:false,message:'Ruten kunne ikke fÃ¸res helt til destinationen',stats};
 const expectedEnd=search.endCell;
 const actualEnd=path[path.length-1];
 if(!expectedEnd||!actualEnd||actualEnd.r!==expectedEnd.r||actualEnd.c!==expectedEnd.c)return{ok:false,message:'Ruten kunne ikke fÃ¸res helt til destinationen',stats};
 const endpointMaxNm=routeEndpointMaxNm();
 if(startDist>endpointMaxNm||endDist>endpointMaxNm)return{ok:false,message:'Ruten kunne ikke fÃ¸res helt til destinationen',stats};
 for(let i=1;i<path.length;i++){
  const dr=Math.abs(path[i].r-path[i-1].r),dc=Math.abs(path[i].c-path[i-1].c);
  if(dr>1||dc>1||(!dr&&!dc))return{ok:false,message:'Ruten blev afvist: route point-kÃ¦den er ikke sammenhÃ¦ngende.',stats};
  if(!cellNavigable(path[i].r,path[i].c,opts))return{ok:false,message:'Ruten blev afvist: validering fandt land/ukendt eller for lav dybde pÃ¥ ruten.',stats};
 }
 stats.routeComplete=true;
 return{ok:true,stats};
}
...
function validateWaterPathDetailed(points,opts){
 if(!Array.isArray(points)||points.length<2)return{ok:false,stats:invalidSegmentStats({index:null,a:null,b:null,point:null,depth:NaN,reason:'for fÃ¥ route points',gridSource:'-'})};
...
function drawRoute(points,depth,label){
 ...
 const tip=label || (Number.isFinite(depth)?`Trollingrute Â· DDM ${depth.toFixed(1)}m`:'Fri navigationsrute');
...
function clearComputedRouteOnly(){
 ...
 if($('routeLength'))$('routeLength').textContent='â€”';
 updateInfoBox();updateRouteActionButtons();updateRouteDebugUi();
}
...
function startTrolling(){if(state.currentRoute?.mode!=='trolling')return setStatus('VÃ¦lg eller lav en trollingrute fÃ¸rst.');startNavigation();}
function stopNavigation(silent=false){state.navActive=false;updateRouteActionButtons();$('navNext').textContent='â€”';$('navXte').textContent='â€”';updateInfoBox();drawForwardLine(Number.isFinite(state.lastCog)?state.lastCog:NaN);if(!silent)setStatus('Navigation stoppet.')}
function updateNavigation(){if(!state.gps||!state.currentRoute?.points?.length)return;const pts=state.currentRoute.points;let near=nearestIndex(pts,state.gps);let nextIdx=Math.min(pts.length-1,near.i+Math.max(3,Math.round(pts.length/30)));let next=pts[nextIdx];const distNext=directNm(state.gps,next);const xte=near.d;const brg=bearingDeg(state.gps,next);$('navNext').textContent=`${distNext.toFixed(2)} NM Â· ${Math.round(brg)}Â°`;$('navXte').textContent=`${xte.toFixed(2)} NM`;checkNavigationAlarms(xte);updateInfoBox();drawForwardLine(Number.isFinite(state.lastCog)?state.lastCog:brg);if(map.getZoom()>=12)map.panTo(state.gps,{animate:true,duration:.25});applyMapOrientation(false)}
function drawForwardLine(heading){
 state.forwardLayer.clearLayers();
 if(!state.gps||!Number.isFinite(heading)){$('navAhead').textContent='Venter pÃ¥ GPS-kurs';return;}
 ...
 for(const nm of [1,2,3]){
  ...
  const depthText=Number.isFinite(d)?` Â· ${d.toFixed(1)} m DDM`:'';
  parts.push(min?`${nm}NM ${min}m`:`${nm}NM`);
  L.circleMarker([p.lat,p.lng],{radius:5,color:'#ff1f2d',weight:3,fillColor:'#fff',fillOpacity:1}).bindTooltip(`<span class="course-label-text">${nm} NM${min?` Â· ca. ${min} min`:''}${depthText}</span>`,{permanent:true,direction:'right',offset:[8,0],className:'course-distance-label'}).addTo(state.forwardLayer)
 }
 $('navAhead').textContent=parts.join(' Â· ')
}
...
function checkNavigationAlarms(xte){
 ...
 if(notes.length)setStatus(notes.join(' Â· '));
}

function updateInfoBox(){
 const heading=Number.isFinite(state.lastCog)?`${Math.round(state.lastCog)}Â° COG`:'â€”';
 const sog=Number.isFinite(state.lastSogKn)?state.lastSogKn:NaN;
 const speed=Number.isFinite(sog)?`${sog.toFixed(1)} kn`:'â€”';
 let eta='â€”';
...
function formatEta(nm,sogKn){
 if(!Number.isFinite(nm)||!Number.isFinite(sogKn)||sogKn<=0.3)return 'â€”';
...
function savedRouteHtml(r,allowDelete=false){
 const meta=r.mode==='free'?`Fri navigation Â· mÃ¥l ${r.depth??'â€”'}m Â· min ${r.minDepth??'â€”'}m Â· ${Number(r.lengthNm||0).toFixed(2)} NM`:`TrollingMode Â· mÃ¥l ${r.depth}m Â· DDM ${Number(r.actualDepth||r.depth).toFixed(1)}m Â· ${Number(r.lengthNm||0).toFixed(2)} NM`;
 return `<strong>${escapeHtml(r.name||'Rute')}</strong><small>${new Date(r.created).toLocaleString('da-DK')} Â· ${meta}</small><div class="savedActions"><button data-act="load">Vis</button><button data-act="nav">NavigÃ©r</button>${allowDelete?'<button data-act="del">Slet</button>':''}</div>`;
}
async function loadSavedRoute(r,startNow=false){
 ...
 if(!await prepareRoutingGrid(start,end))return handleRoutingFailure(state.lastTileLoadError||'Manglende DDM tile-data pÃ¥ gemt rute.',{routingMode:r.mode||'saved',routingSource:'DDM grid',routingTileCount:state.routingTileIds.size,fallbackUsed:false});
 ...
 drawRoute(r.points,Number(r.actualDepth||r.depth),r.mode==='free'?`Fri navigationsrute Â· mÃ¥l ${r.depth??'â€”'}m`:undefined);
...
function savedRouteValidationOptions(r){
 const minDepth=Number.isFinite(Number(r.minDepth))?Number(r.minDepth):(r.mode==='free'?Number(r.depth||freeMinDepth()):0.4);
...
async function cleanupOldCaches(){if(!('caches' in window)) return;try{const keys=await caches.keys();await Promise.all(keys.filter(k=>k.startsWith('waternav-') && k!=='waternav-v45.7').map(k=>caches.delete(k)))}catch(e){console.warn('Cache cleanup failed',e)}}
