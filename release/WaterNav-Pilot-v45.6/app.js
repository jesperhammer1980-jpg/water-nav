...
init();
function init(){migrateUserData();bindUI();loadUserSettings();loadSavedRoutes();renderSavedRoutes();loadHomePort();loadOrientationPreference();updateManifestDependentControls();loadTileManifest().then(()=>updateVisibleMapTiles({initial:true})).then(()=>setStatus('v45.6 klar. DDM tiles loader dynamisk for synligt kortområde og ruter.')).catch(e=>{console.error(e);setStatus(manifestErrorMessage(e));});updateInfoBox();cleanupOldCaches();if('serviceWorker'in navigator)navigator.serviceWorker.register('./sw.js?v=45.6').then(r=>r.update()).catch(()=>{});setTimeout(()=>{map.invalidateSize(true);applyMapOrientation(true);scheduleVisibleTileUpdate();},300)}
function bindUI(){
...
function loadUserSettings(){
 try{const s=JSON.parse(localStorage.getItem(USER_SETTINGS_KEY)||'{}');state.depthAlarm=s.depthAlarm!==false;state.offRouteAlarm=s.offRouteAlarm!==false;state.routeSmoothing=ROUTE_SMOOTHING_VALUES.has(s.routeSmoothing)?s.routeSmoothing:'normal';}catch{state.depthAlarm=true;state.offRouteAlarm=true;state.routeSmoothing='normal'}
...
function importUserData(evt){
 ...
 if(data.userSettings){state.depthAlarm=data.userSettings.depthAlarm!==false;state.offRouteAlarm=data.userSettings.offRouteAlarm!==false;state.routeSmoothing=ROUTE_SMOOTHING_VALUES.has(data.userSettings.routeSmoothing)?data.userSettings.routeSmoothing:state.routeSmoothing;saveUserSettings();loadUserSettings();}
...
function setRouteSmoothing(mode,silent=false){
 state.routeSmoothing=ROUTE_SMOOTHING_VALUES.has(mode)?mode:'normal';
 updateRouteSmoothingUi();
 saveUserSettings();
 if(!silent)setStatus(`Ruteudjævning: ${routeSmoothingLabel(state.routeSmoothing)}.`);
}
function updateRouteSmoothingUi(){
 document.querySelectorAll('input[name=routeSmoothing]').forEach(el=>{el.checked=el.value===state.routeSmoothing});
}
...
function buildWaterRoute(start,end,opts){
 ...
 if(!validateWaterPath(rawPts,opts))return{ok:false,message:'Ruten blev afvist: validering fandt land/ukendt eller for lav dybde på et ruteben.',stats:baseStats};
...
function applyRouteSmoothing(points,opts){
 const mode=state.routeSmoothing||'normal';
...
function validateSmoothedRouteOutput(start,end,points,opts){
 ...
 if(!validateWaterPath(points,opts))return{ok:false,message:'Ruteudjævning blev afvist: segment krydser land, for lav dybde eller manglende DDM-data.',stats};
...
function clearRoute(){
 ...
 setRoutingDebug('Rute ryddet',null,0,0,{originalPointCount:0,smoothedPointCount:0,smoothingReductionPct:0,smoothingSplineUsed:false,smoothingFallback:false});
...
async function cleanupOldCaches(){if(!('caches' in window)) return;try{const keys=await caches.keys();await Promise.all(keys.filter(k=>k.startsWith('waternav-') && k!=='waternav-v45.6').map(k=>caches.delete(k)))}catch(e){console.warn('Cache cleanup failed',e)}}
