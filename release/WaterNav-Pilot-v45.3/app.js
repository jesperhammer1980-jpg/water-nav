...
init();
function init(){migrateUserData();bindUI();loadUserSettings();loadSavedRoutes();renderSavedRoutes();loadHomePort();loadOrientationPreference();updateManifestDependentControls();loadTileManifest().then(()=>updateVisibleMapTiles({initial:true})).then(()=>setStatus('v45.3 klar. DDM tiles loader dynamisk for synligt kortområde og ruter.')).catch(e=>{console.error(e);setStatus(manifestErrorMessage(e));});updateInfoBox();cleanupOldCaches();if('serviceWorker'in navigator)navigator.serviceWorker.register('./sw.js?v=45.3').then(r=>r.update()).catch(()=>{});setTimeout(()=>{map.invalidateSize(true);applyMapOrientation(true);scheduleVisibleTileUpdate();},300)}
function bindUI(){
...
function setStart(p){state.start={lat:p.lat,lng:p.lng};if(state.startMarker)state.startMarker.setLatLng(state.start);else state.startMarker=L.marker(state.start,{icon:startIcon}).addTo(map).bindPopup('Start');setStatus('Start valgt. Vælg slutpunkt.')}
function setEnd(p){state.end={lat:p.lat,lng:p.lng};if(state.endMarker)state.endMarker.setLatLng(state.end);else state.endMarker=L.marker(state.end,{icon:endIcon}).addTo(map).bindPopup('Slut');setStatus(state.trollingEnabled?'Slut valgt. Lav trollingrute på DDM-dybdekurven.':'Slut valgt. Lav fri rute efter valgt dybde.')}
...
async function prepareRoutingGrid(start,end){
 ...
 if(!pointInsideGrid(start,state.depthGrid)||!pointInsideGrid(end,state.depthGrid))return false;
...
function makeRoute(){if(!requireTileManifest('Lav rute'))return;setStatus('Henter DDM tiles og beregner vandrute...'); setRouteBusy(true); setTimeout(async()=>{try{state.trollingEnabled?await makeTrollingRoute():await makeFreeRoute()}catch(e){console.error(e);setStatus('Ruteberegning fejlede: '+(e?.message||e))}finally{setRouteBusy(false);updateRouteActionButtons()}},30)}
...
async function makeTrollingRoute(){
 if(!state.start||!state.end)return setStatus('Vælg start og slut først.');
 if(!requireTileManifest('Lav trollingrute'))return;
 if(!await prepareRoutingGrid(state.start,state.end))return setStatus(state.lastTileLoadError||'Start/slut ligger uden for det installerede DDM tile-område.');
...
function buildWaterRoute(start,end,opts){
 ...
 if(!path||path.length<2)return{ok:false,message:`Kunne ikke finde en DDM-water-only rute med minimum ${Number(opts.minDepth||0).toFixed(1)} m. Prøv lavere minimumsdybde eller flyt start/slut til dybere vand.`};
...
function drawRoute(points,depth,label){updateInfoBox();if(state.routeLayer)map.removeLayer(state.routeLayer);const tip=label || (Number.isFinite(depth)?`Trollingrute · DDM ${depth.toFixed(1)}m`:'Fri navigationsrute');state.routeLayer=L.polyline(points.map(p=>[p.lat,p.lng]),{color:'#20d8ff',weight:6,opacity:.95,lineCap:'round'}).addTo(map).bindTooltip(tip,{sticky:true});map.fitBounds(state.routeLayer.getBounds(),{padding:[60,60]});updateRouteActionButtons()}
function clearRoute(){
 stopNavigation(true);
...
async function cleanupOldCaches(){if(!('caches' in window)) return;try{const keys=await caches.keys();await Promise.all(keys.filter(k=>k.startsWith('waternav-') && k!=='waternav-v45.3').map(k=>caches.delete(k)))}catch(e){console.warn('Cache cleanup failed',e)}}
