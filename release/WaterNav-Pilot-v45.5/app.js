...
init();
function init(){migrateUserData();bindUI();loadUserSettings();loadSavedRoutes();renderSavedRoutes();loadHomePort();loadOrientationPreference();updateManifestDependentControls();loadTileManifest().then(()=>updateVisibleMapTiles({initial:true})).then(()=>setStatus('v45.5 klar. DDM tiles loader dynamisk for synligt kortområde og ruter.')).catch(e=>{console.error(e);setStatus(manifestErrorMessage(e));});updateInfoBox();cleanupOldCaches();if('serviceWorker'in navigator)navigator.serviceWorker.register('./sw.js?v=45.5').then(r=>r.update()).catch(()=>{});setTimeout(()=>{map.invalidateSize(true);applyMapOrientation(true);scheduleVisibleTileUpdate();},300)}
function bindUI(){
...
function buildWaterRoute(start,end,opts){
 ...
 const pts=path.map(cellToLatLng);
 const validation=validateCompleteRouteOutput(start,end,path,pts,opts,search);
 if(!validation.ok)return{ok:false,message:validation.message,stats:{...baseStats,...validation.stats}};
 if(!validateWaterPath(pts,opts))return{ok:false,message:'Ruten blev afvist: validering fandt land/ukendt eller for lav dybde på et ruteben.',stats:baseStats};
 ...
 return{ok:true,points:pts,avgDepth,stats:{...baseStats,...validation.stats,minDepth,maxDepth,avgDepth,gridSteps:path.length,startDepth:depthAtCell(sCell.r,sCell.c),endDepth:depthAtCell(eCell.r,eCell.c)}};
}
...
async function cleanupOldCaches(){if(!('caches' in window)) return;try{const keys=await caches.keys();await Promise.all(keys.filter(k=>k.startsWith('waternav-') && k!=='waternav-v45.5').map(k=>caches.delete(k)))}catch(e){console.warn('Cache cleanup failed',e)}}
